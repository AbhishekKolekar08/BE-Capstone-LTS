...
% open VehicleModelv1.m;
open TrackInput.m;
trackfile = 'Track Data/Track_Spa-Francorchamps.mat' ;
vehiclefile = 'Vehicles Data/VEHICLE_Formula 1_Open Wheel.mat' ;
...
simname = "Lap Sim/"+char(veh.name)+"_"+tr.info.name+date_time ;
logfile = simname+".log" ;

%% HUD

[folder_status,folder_msg] = mkdir('Lap Sim') ;
delete(simname+".log") ;
...
disp(['Power Consumption:  ',num2str(sim.total_power_consumption.data,'%3.3f'),' [kWh]'])
...
% legend('Lap','GGV Limit','Location','northeast')
% xlabel('LatAcc [m/s^2]')
% ylabel('LonAcc [m/s^2]')
% zlabel('Speed [km/h]')
% grid on
...
legend('Velocity on Track (km/hr)','Location','northeast')
xlabel('X [m]')
ylabel('Y [m]')
colorbar
grid on
...
f4 = figure;
hold on
plot(tr.x,sim.power_consumption.data)
xlabel('Distance [m]')
ylabel('Power Consumed [kWh]')
...
function [sim] = simulate(veh,tr,simname,logid)
    ...
    engine_torque = TPS.*interp1(veh.vehicle_speed,veh.tractive,V,'linear','extrap') ;
    engine_power = TPS.*interp1(veh.vehicle_speed,veh.powerVar,V,'linear','extrap') ;
    engine_speed = interp1(veh.vehicle_speed,veh.RPMVar,V,'linear','extrap') ;
    %gear = interp1(veh.vehicle_speed,veh.gear,V,'nearest','extrap') ;
    power_consumption = (engine_power/(50*3600));
    power_consumption_cumulative = cumsum(power_consumption) ;
    power_consumption_total = power_consumption_cumulative(end) ;
    % HUD
...
%     [~,i] = max(abs(AY)) ;
    ...
    sim.power_consumption.data = power_consumption;
    sim.power_consumption.unit = 'kW';
    sim.total_power_consumption.data = power_consumption_total;
    sim.total_power_consumption.unit = 'kW';
%     sim.gear.data = gear ;
%    sim.gear.unit = [] ;
    ...
    if r==0 % straight (limited by engine speed limit or drag)
        ...
        while adjust_speed
            ...
            if ax_drag<=0 % need throttle to compensate for drag
                ...
                ax_power_limit = 1/M*(interp1(veh.vehicle_speed,veh.factor_power*veh.fx_motor,v)) ;
...
function [v_next,ax,ay,tps,bps,overshoot] = vehicle_model_comb(veh,tr,v,v_max_next,j,mode)
    ...
    if ax_needed>=0 % need tps
        ...
        % getting power limit from engine
        ax_power_limit = 1/M*(interp1(veh.vehicle_speed,veh.factor_power*veh.fx_motor,v,'linear',0)) ;
        % getting tps value
