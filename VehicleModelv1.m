...
% motor
peak_torque = str2double(table2array(info(i,2))) ; i = i+1 ;
peak_power = str2double(table2array(info(i,2))) ; i = i+1 ;
peak_RPM = str2double(table2array(info(i,2))) ; 


%% HUD

[folder_status,folder_msg] = mkdir('Vehicles Data') ;
vehname = "Vehicles Data/VEHICLE_"+name+"_"+type ;
delete(vehname+".log") ;
diary(vehname+".log") ;
disp('====================================================================================')
disp(filename)
...
br_pist_a = br_nop*pi*(br_pist_d/1000)^2/4 ; % Brake piston area [m2]
br_mast_a = pi*(br_mast_d/1000)^2/4 ; % Master cylinder area [m2]
...
% Speed Torque Power Curve Generation
for RPM=0:deltaRPM:peak_RPM
    if power < peak_power
        torque = peak_torque;
        power = (2*3.142*RPM*torque)/(60*1000);
    else
        power = peak_power;
        torque = (power*60*1000)/(2*3.142*RPM);
    end
    RPMVar(j) = RPM;
    torVar(j) = torque;
    powerVar(j) = power;
...
% Wheel RPM and Torque Calculation
wheel_speed = RPMVar/ratio_final ;
vehicle_speed = wheel_speed*2*pi/60*tyre_radius ;
wheel_torque = torVar*ratio_final*n_final ;
fx_motor1 = wheel_torque./tyre_radius;
fx_motor = fx_motor1';
...
% dv = 0.5/3.6 ;
vehicle_speed = linspace(v_min,v_max,length(vehicle_speed))' ;

fx = zeros(length(vehicle_speed),1) ;
...
tractive = factor_power.*fx_motor;
minimum =  cat(2,tractive,fx_tyre);
final_tractive = min(minimum,[],2);
...
for i=1:length(v)
    ...
    ax_power_limit = 1/M*(interp1(vehicle_speed,factor_power*fx_motor1,v(i))) ;
...
subplot(rows,cols,1)
hold on
title('Motor - Speed Torque Power Characteristics')
yyaxis left;
plot(RPMVar,wheel_torque,'linewidth',1);
xlabel('Motor RPM');
ylabel('Wheel Torque (Nm)');
yyaxis right
plot(RPMVar,powerVar,'linewidth',1);
ylabel('Power (kW)');
...
h1 = plot(vehicle_speed,tractive,'k','LineWidth',2);
h2 = plot(vehicle_speed,fx_tyre,'c','LineWidth',2);
h3 = plot(vehicle_speed,-fx_aero,'y','LineWidth',2);
h4 = plot(vehicle_speed,-fx_roll,'m','LineWidth',2);
h5 = plot(vehicle_speed,final_tractive,'r','LineWidth',2);
grid on
xlabel('Speed [m/s]')
ylabel('Force [N]')
xlim([0,max(vehicle_speed)]);
legend([h1(1),h2(1),h3(1),h4(1),h5(1)],{'Tractive force','Max force at tyre','Aero drag','Rolling resistance','Effective Tractive Force'},'Location','southoutside')
